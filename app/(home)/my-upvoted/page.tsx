      ...
      {products.length === 0 ? (
        <div className="">
          <h1 className="text-3xl font-bold">
            You have not upvoted any product yet
          </h1>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
            {products.map((product: any) => (
              <Link href={`/product/${product.slug}`} key={product.id}>
                <div>
                  <div className="rounded-lg cursor-pointer hover:scale-105 transition-transform duration-300 ease-in-out justify-center items-center border">
                    <Image
                      src={product.logo}
                      alt="logo"
                      height={1000}
